...
def send_otp_email(target_email, otp_code):
    try:
        ...
        msg['To'] = target_email
        msg['Subject'] = "Your Verification OTP: " + str(otp_code) + " - CaptionVFX Studio"
        ...
        return False, "Email send failed: " + str(e)
    ...
    with col_center:
        ...
        with auth_tab2:
            ...
            if st.session_state["generated_otp"]:
                st.info("📧 Verification OTP sent to: `" + st.session_state['otp_target_email'] + "`")
...
def format_ass_time(seconds):
    ...
    return "{:d}:{:02d}:{:02d}.{:02d}".format(hours, minutes, secs, centisecs)
...
# ==================== 4. SIDEBAR CONTROLS ====================
st.sidebar.markdown("📧 **Verified User:** `" + st.session_state['user_email'] + "`")
    ...
    if render_clicked:
        with st.spinner("⚡ AI processing subtitles & rendering video..."):
            ...
            if auto_vibe:
                ...
                st.success("🎯 AI Detected Video Genre: **" + vibe_info['name'] + "**")
                effect_tag = "{\\c" + active_color + vibe_info["tag"][1:]
